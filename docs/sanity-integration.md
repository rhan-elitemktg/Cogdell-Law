...
| `pageBody` (Portable Text) | — | object | — | `[x]` | `[x]` | `[x]` | ✅ done — `blockContent` + insertable layout blocks; **`practiceArea.body` + `locationPage.body` only (D16)** |
| `bodyCta` | — | object | — | `[x]` | `[x]` | `[x]` | ✅ done — CTA Banner block; buttons default in code from Firm Details (D16) |
...
### F23 — `options.insertMenu` does nothing on a Portable Text field

Found while building the body CTA block (D16). Sanity has **two** insert menus, and only one
of them reads your config:

- The **array-of-objects** input passes `schemaType.options?.insertMenu` into
  `@sanity/insert-menu`, so `views`, `groups` and the filter all work.
- The **Portable Text** toolbar builds its own menu with a local
  `getInsertMenuItems(types, …)` that takes only the member list. `options.insertMenu` is
  never read on that path.

So on `pageBody` there is nothing to configure. The levers that *do* work are each member
type's **`icon`** and **`title`**, plus its `preview.prepare()` for the collapsed row. With
five block types the icons are the entire UX budget, which is why D16 reserves one per block
up front. Don't spend an afternoon on `insertMenu` here — and re-confirm this when block #2
lands, in case a later Studio release wires it up.

Two related mechanics worth writing down:

- **Block-level vs inline is decided by where the member is declared**, not by a flag. Members
  of the array are block-level; members of the `block` member's `of:` are inline. `pageBody`
  therefore omits `of:` entirely — the omission *is* the guarantee that a banner can't land
  mid-sentence.
- **Drag-to-reorder is free.** Sanity renders block objects `draggable` with a drop indicator;
  no config, nothing to build. Insertion happens at the caret from the toolbar, so editors
  insert-then-drag.
...
| **D18** | "Reviewed By" rail panel | ✅ **Set 2026-07-29.** Heads the practice-area rail: circular attorney photo left, "Reviewed By" over "&lt;name&gt; — Published/Updated on &lt;date&gt;" right. The name links to `/attorney/<slug>`. <br><br>**The reviewer resolves in three steps** — the page's own `reviewedBy`, then Firm Details → **Default reviewer**, then the first attorney in Attorney Bios. Step 3 is a zero-config floor, not the resting state: it follows that list's drag order, so reordering attorneys would change who is credited. Set the default in Firm Details to pin it. <br><br>**Published vs Updated is decided on the FORMATTED dates, not the timestamps.** Every published document is written at least twice — creating and publishing are separate mutations — so `_updatedAt` is essentially never equal to `_createdAt`, and comparing instants would label every page "Updated" from the moment it went live. Comparing rendered strings also keeps the label honest about what the reader can see: if both render "June 4, 2026", calling it an update is a distinction with no visible difference. Verified: a page created today reads **Published on**, pages migrated on 2026-07-16 and edited since read **Updated on**. <br><br>Caveat worth knowing: `_updatedAt` moves on *any* mutation, including a bulk script, so a migration will flip pages to "Updated" whether or not their words changed. <br><br>**Practice areas only.** `Rail` takes an optional `pageId`; passing it heads the rail with the panel, omitting it leaves the rail opening on the form as before. Location pages don't pass one — they have no `reviewedBy` field, so they'd have nothing to override the default with. Adding them is that field plus one prop. |
| **D17** | Fact-checked banner | ✅ **Set 2026-07-29.** A trust banner at the foot of every practice area and location page — tick, badge, and a statement naming the editorial guidelines and the approving attorney. Full container width, **outside the body/rail grid**, so it runs under both columns. <br><br>**One shared record + a per-page override + a per-page toggle** — the `ctaBar` arrangement from D13, with a switch added. `factCheckContent` is the shared shape; the `factCheck` singleton (Site Settings) holds the default; each page carries a collapsed **Fact-Checked Banner** section, last on the page so it reads as a setting rather than content. `factCheckField()` builds that section for both document types from one factory, so they can't drift — only the field group differs. <br><br>**ON BY DEFAULT, AND `initialValue` ALONE CANNOT DELIVER THAT.** It fires only for newly created documents, so all ~26 practice areas and location pages that predate the field carry no `factCheck` object at all. The query therefore reads `coalesce(*[…].factCheck.show, true)` and the renderer treats only an explicit `false` as off. **Note this can't be written `show != false`** — in GROQ a comparison against null yields null, not true. <br><br>**Renders nothing when the singleton has no statement**, rather than throwing the way `CtaBar` does: the toggle ships on for every existing page, so a hard failure would take the whole build down until someone seeded it. Seed with `npx sanity exec scripts/seed-fact-check.ts --with-user-token`. <br><br>The statement is `blockContent` because it needs **links** (editorial guidelines, the attorney) — the same reason `ctaBarContent.body` is. |
| **D16** | Body blocks (`pageBody`) | ✅ **Set 2026-07-28.** `practiceArea.body` and `locationPage.body` moved from `blockContent` to **`pageBody`** — the same text toolbar plus **block-level layout objects an editor can insert between paragraphs and drag into place**. First block: **`bodyCta`** (CTA Banner). <br><br>**A superset, not a replacement.** `pageBody.of` is `[bodyBlockMember(), {type: "bodyCta"}]`, and `blockContent.of` is `[bodyBlockMember()]` — the *same factory* in `blockContent.ts`, so the text toolbar is identical by construction and cannot drift. See the D12 superset clause. <br><br>**Scope is the point.** Only those two page types — they own a full content column. `legalPage`, `attorney.bio`, FAQ answers, news, podcast and every band body stay on `blockContent` and get **no Insert control at all**. A CTA banner has no business inside an FAQ answer. <br><br>**Zero migration.** A field's *type name* is a schema alias and is not stored in the document; every existing member is `_type: "block"`, which `pageBody` declares. No writes, no `_rev` churn, no `_updatedAt` churn (so sitemap lastmods don't move). Verified: after the flip, the only change in `sanity.types.ts` was four `body: BlockContent` → `body: PageBody` lines. <br><br>**Rollback caveat:** flipping back to `blockContent` is equally free *until* a document holds a `bodyCta`. After that, a rollback leaves that member unresolvable and the Studio shows an "Unknown type" chip — data preserved, not destroyed. <br><br>**Block-level only.** Members sit beside the text member, never in its `of:` (that's Sanity's inline-object slot). See F23. <br><br>**Buttons resolve in code, never in the document.** `bodyCta.primaryCta`/`secondaryCta` are optional; empty means the renderer supplies "Schedule Consultation" → `/contact` and "Call Us &lt;phone&gt;" with the number read from **Firm Details at build time**. Deliberately **no `initialValue`** — that snapshots, so a later phone change would strand every banner already placed on the old number. Cost: an empty button field gives the editor no hint, paid off by the field `description`s and a preview subtitle reading "Standard buttons". <br><br>Required this: **`getFirmDetails()` is now memoized** (promise cached, PROD only) in `src/sanity/lib/firmDetails.ts`. Layout, Header, Footer and ContactMethods already each called it, so it was 3–4+ identical round-trips per page across ~45 pages with `useCdn: false`. <br><br>**Block 2 — `bodyPhoneBar` (Get In Touch Bar), 2026-07-28.** Quieter than `bodyCta`: heading left, small label + phone number right, no buttons. `eyebrow` falls back to "Confidential & Privileged"; `phone` falls back to Firm Details, with an **override field** that exists for location pages advertising their own local line. Its right-aligned eyebrow uses `rule={false}` — already the house pattern for eyebrows that aren't left-aligned (`FirmStory`, `WhyChoose`, `PodcastHero`), since a leading rule would point away from the edge it sits against. <br><br>**Block 3 — `bodyAttorney` (Attorney Card), 2026-07-29.** Photo left, name + a short passage + the same two buttons right. The attorney is a **reference**, so name and photo come from their record and a new headshot lands everywhere at once; the passage stays on the block, because that copy is about *this page*, not about the person. <br><br>**This is the block that triggered the dereference noted above.** Both queries now project `body[]{ ..., _type == "bodyAttorney" =&gt; { attorney-&gt;{ name, photo, photoAlt } } }` instead of taking `body` raw — verified: TypeGen resolves `attorney` to the full object and the other three members still pass through the `...` spread untouched. **The projection is duplicated in `practiceAreas.ts` and `areasWeServe.ts` on purpose**; change one, change the other. Any future block holding a reference needs its own `_type ==` arm in both. <br><br>Two smaller calls: its Studio preview is **text-only, no `media`** — selecting an image into a preview is what crashed the Podcast Episodes list (React 19 ref loop, see `podcast.ts`), and while that was a document list rather than an array item, a thumbnail wasn't worth risking the body editor on an untested hunch. And on mobile it stacks **left-aligned**, unlike its two siblings, because centred body copy is harder to read than a centred heading and phone number. <br><br>The mockup drew its secondary button as orange-on-navy; the firm chose to **keep the existing white `.btn--outline`** rather than add a variant, so all three blocks share one button treatment. <br><br>**Block 4 — `bodyQuoteCta` (Attorney Quote), 2026-07-29.** Accent quote mark, the passage in the attorney's own voice, attribution, the same two buttons, photo right. Sibling of `bodyAttorney` and easy to confuse with it — that block is a third-person introduction, this one quotes them; same reference, different voice. Marked up as `figure > blockquote + figcaption`. Portrait 9:10 crop rather than `bodyAttorney`'s square, and the attribution is `--cog-cream` rather than `QuoteBand`'s `--cog-navlink`, both per the mockup. Photo sits **first in the DOM** and moves right via `order`, so the stacked layout leads with it — the F22 arrangement. <br><br>**Watch out: don't merge the two dereference arms.** `_type in ["bodyAttorney", "bodyQuoteCta"] =&gt; {…}` reads better and returns identical data, but **TypeGen can't prove it applies** — it then emits both a dereferenced *and* a raw-`_ref` variant per block, widening `attorney` to `AttorneyReference | {…}` so every renderer has to narrow it. Tried, measured, reverted. **One `_type ==` arm per type**, even when the projection is word-for-word the same. Noted in both query files. <br><br>**Block 5 — `bodyTestimonial` (Client Testimonial), 2026-07-29.** The odd one out: a **paper card on the page's own paper**, not a navy interruption, because a review is evidence rather than a call to action. Borrows the blockquote's lift — both now use a shared **`--shadow-card`** token rather than two literals, so "same shadow as the blockquote" can't quietly stop being true. No accent border: the blockquote's left rule marks quoted body copy, and this is already set apart by being a card. <br><br>**References the existing testimonial pool** (Collections → Testimonials) rather than retyping — the same review already feeds the homepage band and the /testimonials wall, and three copies of one client's words would drift. The block therefore has exactly one field, the reference. <br><br>Added an optional **`reviewedAt`** date to the `testimonial` document for the "3 months ago" line; leave it empty and the line is omitted. Additive and optional, so no migration. <br><br>**Relative time is a deliberate exception** — the rest of the site prints absolute dates (news, podcasts), but a review reads as current when dated the way Google dates one. `src/lib/relativeTime.ts` computes it **at build time**, so the visible string freezes until the next deploy; publishing fires the deploy hook so it refreshes whenever content changes, but a quiet spell lets it drift. **The real date always goes in `<time datetime>`**, so the markup stays correct even when the text is behind. Switch to absolute by dropping `relativeTime()` for `toLocaleDateString`, as `PodcastCard` does. <br><br>Stars are **hardcoded at five** — there's no rating field anywhere on the site, and `Testimonials.astro` and the wall both show five too. Add a `rating` to `testimonial` if that ever needs to vary. <br><br>**Roadmap — one more block:** Key Takeaways (`bulb-outline` reserved). <br><br>**When the attorney/quote blocks land they will hold references**, so the raw `body` projection stops being enough and both queries need `body[]{ ..., _type == "bodyAttorney" =&gt; { attorney-&gt;{…} } }`. TypeGen handles `=&gt;` fine, but `defineQuery()` is parsed **statically**, so that projection **cannot** be hoisted to a shared const — it must be written out inline in `practiceAreas.ts` *and* `areasWeServe.ts`. Duplicated on purpose; don't "clean it up". |
| **D15** | Per-page SEO fields | ✅ **Set 2026-07-23.** One shared `seo` object (`schemaTypes/seo.ts`) attached to all **16** routed documents — the 10 page singletons plus `practiceArea`, `locationPage`, `legalPage`, `newsItem`, `podcast`, `attorney`. (Was 14 when written; `podcast` and `podcastPage` came later and picked the field up.) Five fields: **`metaTitle`, `metaDescription`, `canonicalUrl`, `noIndex`, `ogImage`**. No keywords field (Google ignores the tag) and no separate og title/description (they'd double the field count for a case that rarely differs). <br><br>**Every field is optional and falls back to what the page already rendered** — the resolution lives in `src/lib/seo.ts` (`resolveSeo`), so adding the field to a document is a no-op until an editor fills it in. Verified: with the dataset untouched, all 45 pages' `<title>` and `<meta name="description">` are **byte-identical** to the pre-change build. <br><br>`metaTitle` replaces the page name only — **`\| Cogdell Law` is always appended**, so the SEO team never types the brand. The homepage's bare "Cogdell Law" survives when it has no `metaTitle`. <br><br>Length rules are **`.warning()`, never `.error()`**: publishing fires the Vercel deploy hook, so a blocking validation error over a 62-character title would stop the whole site rebuilding. <br><br>`/site-map` and `/404` have no backing document and keep code-level metadata; `/404` is hardcoded `noIndex`. |
...
**Structured data** — JSON-LD derived from existing content, **no fields to fill**:
`LegalService` sitewide from Firm Details (address parsed out of `cityStateZip`, socials →
`sameAs`), and `FAQPage` from `PracticeFaqs.astro`. No page carries two FAQPages
(`PracticeFaqs` and the homepage's `Faq.astro` never co-occur).

⚠️ **Corrected 2026-08-12: `BreadcrumbList` is no longer emitted.** This originally read
"three JSON-LD blocks… Built: 44 LegalService, 28 BreadcrumbList, 5 FAQPage". Commit `9c6521b`
deleted `Breadcrumb.astro`, which was `breadcrumbSchema()`'s only caller — the function still
exists in `src/lib/schema.ts` with **no callers**, so the markup ships on zero pages. The
`trail` data is still built by `practiceAreas.ts` and `areasWeServe.ts`, so re-wiring it is one
component and one import, not a rebuild.
